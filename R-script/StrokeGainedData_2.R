...
par_file <- read.csv(file.path(bip_path, sprintf("%s_file_par.csv",current_Event)))

final_file <- par_file %>% inner_join(file_sg, by = c("player", "year")) %>% select(player, year, Posn, round_total, Current:Location, sgtot4:sgp_all, par34:Ss_all)
