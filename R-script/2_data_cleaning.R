###### Data preparation ####
setwd("D:/Golf/2022/dubai")
library(readxl)
df <- read_excel("Dubai Desert Classic_NA.xlsx", sheet = "Historic", na = c("", "-", "NaN")) # read the spreadsheet including using na for blank and - spaces
library(janitor)
df <- clean_names(df)
library(dplyr)
df_2022 <- read_excel("Dubai Desert Classic_NA.xlsx",sheet = "Current", na = c("", "-", "NaN"))
