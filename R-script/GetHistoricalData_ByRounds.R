...
par_data_gained <- pariGainedRound(par_Gained, bip)

# get the stroke gained data
stroke_gained <- StrokeGainedData(bip, sg, current_tour)

stroke_data_gained <- StrokeGainedRounds(stroke_gained, bip)


# Join the two dataframes
final_file <- par_data_gained %>% left_join(stroke_data_gained, by = c("player", "year")) %>% select(player, year, Posn.x, round_total, Current:Location, sgtot4:sgp_all, par34:Ss_all)
